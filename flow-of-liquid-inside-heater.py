...
app.title = 'PID - flow of liquid in heater'
...
app.layout = html.Div([
    html.H1(children=['PID - flow of liquid in heater', html.Br(), html.P(id="subtitle", children="Mathematical model of the automatic control system in a flow heater")]),
    html.P(id="description", children="The presented mathematical model of the PID controller allows to set the desired water temperature and heater parameters.  "
                                      "Controlled variable of the system is the voltage of the heater (heating liquid, which is water). Charts of temperature, "
                                      "control signal, voltage and power over time are displayed "
    ...
    html.Div(id="block", children=[
        html.Div(className='sliders', children=["Select the temperature and parameters of the heater", html.Br(),
            ...
            html.Div(id='loading', children=[
                dbc.Button("Show chart", id='submit-button-state', n_clicks=0),
                dbc.Alert("Values out of range have been entered! Fields marked with a red frame should be corrected.", color="danger", id="alert-auto", is_open=False, duration=10000)
            ])
        ]),

    #legenda
    html.Div(className="legend", children=[html.P(id="legend_title", children=["Legend"]),
        "T - temperature", html.Br(), "P - heater power", html.Br(), "V - boiler volume", html.Br(), "Tw - initial water temperature", html.Br(), "dw - water inflow rate", html.Br(), "Umax - max. voltage on the heater", html.Br(),
        "t - simulation time", html.Br(), "Tp - sampling period", html.Br(), "kp - proportional gain", html.Br(), "Ti - integral time", html.Br(), "Td - dertivative gain"
    ]),
    ...
    #podgrzewacz 2
        html.Div(className='sliders', children=["Select the temperature and parameters of the heater:", html.Br(),
            ...
            html.Div(id='loading1', children=[
                dbc.Button("Show chart", id='submit-button-state1', n_clicks=0),
                dbc.Alert("Values out of range have been entered! Fields marked with a red frame should be corrected.", color="danger", id="alert-auto1", is_open=False, duration=10000)
            ])
        ]),
    ]),
    ...
    html.Div(className="regulation_block", children=[
        html.Div(className="regulation", children=[
            "Adjustment time: ", html.Output(id="czas"), html.Br(),
            "Overshoot: ", html.Output(id="przeregulowanie"), html.Br(),
            "Difference: ", html.Output(id="uchyb"), html.Br(),
            "Accuracy of regulation: ", html.Output(id="dokladnosc"), html.Br(),
            "Costs of regulation: ", html.Output(id="koszty")
        ]),
    ]),

    html.Div(className="regulation_block", children=[
        html.Div(className="regulation", children=[
            "Adjustment time: ", html.Output(id="czas1"), html.Br(),
            "Overshoot:  ", html.Output(id="przeregulowanie1"), html.Br(),
            "Difference ", html.Output(id="uchyb1"), html.Br(),
            "Accuracy of regulation: ", html.Output(id="dokladnosc1"), html.Br(),
            "Costs of regulation: ", html.Output(id="koszty1")
        ]),
    ]),
    ...
    html.P(id="footer", children=["2021 © Wiktor Jankowski, Patryk Jędrzejewski (back-end), Filip Kaczmarek (frontend)"])
])
...
def update_output(n_clicks, is_open, selected_T, selected_P, selected_V, selected_Tw, selected_dw, selected_kp, selected_Ti, selected_Td, selected_Umax, selected_t, selected_Tp):
    ...
    plot = make_subplots(rows=4, cols=1, subplot_titles=(
        "Temperature over time - T(t)", "Control signal (controlling variable) over time - u(t)",
        "Voltage of heater (controlled variable) over time - U(t)", "Power of heater over time - P(t)"))
...
def update_output(n_clicks, is_open, selected_T, selected_P, selected_V, selected_Tw, selected_dw, selected_kp, selected_Ti, selected_Td, selected_Umax, selected_t, selected_Tp):
    ...
    czas_regulacji = czas_regulacji * Tp / 60
    print("Regulation time ", czas_regulacji)
    czas_regulacji = round_num(czas_regulacji)
    czas_regulacji = str(czas_regulacji + " min")

    przeregulowanie = (max(T) - T_zadane) / T_zadane * 100
    print("Overshoot", przeregulowanie, "%")
    przeregulowanie = round_num(przeregulowanie)
    przeregulowanie = str(przeregulowanie + " %")

    uchyb_ustalony = e[-1]
    print("Difference", uchyb_ustalony)
    uchyb_ustalony = round_num(uchyb_ustalony)

    dokladnosc_regulacji = Tp * sum(map(abs, e))
    print("Regulation accuracy indicator", dokladnosc_regulacji)
    dokladnosc_regulacji = round_num(dokladnosc_regulacji)

    koszty_regulacji = Tp * sum(map(abs, u))
    print("Regulation costs indicator", koszty_regulacji)
    koszty_regulacji = round_num(koszty_regulacji)
